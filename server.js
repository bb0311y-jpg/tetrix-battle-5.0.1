...
function handlePieceLock(room, board, opponentBoard, playerIndex) {
  try {
    ...
    console.error(`handlePieceLock error for player ${playerIndex}:`, error);
    return { gameEnded: false };
  }
}
    ...
    if (room && room.players.size === 2) {
      ...
      room.gameLoopInterval = setInterval(() => {
        try {
          ...
          if (room.pieceIndex >= room.pieceSequence.length - 10) {
            console.warn(`Room ${roomId}: Piece sequence running low, generating more...`);
            for (let i = 0; i < 500; i++) {
              const type = PIECE_TYPES[Math.floor(room.rng.next() * PIECE_TYPES.length)];
              room.pieceSequence.push(type);
            }
          ...
          console.error(`Game loop error in room ${roomId}:`, error);
          clearInterval(room.gameLoopInterval);
          io.to(roomId).emit('gameError', { message: '遊戲發生錯誤，已結束' });
        }
      }, 50);
